...
class DPAdapterTrainer:
    ...
    def _create_val_loader(self):
        """创建MNLI验证集"""
        # 加载验证集（MNLI有匹配和不匹配两个验证集）
        val_dataset = load_dataset(
            "glue", "mnli", 
            split="validation_matched",  # 使用匹配验证集
    ...
    def train(self):
        ...
        for step in range(self.total_steps):
            ...
            if (step+1) % eval_interval == 0:
                # 评估匹配和不匹配两个验证集
                matched_loss, matched_acc = self.evaluate_model()
                
                print(f"\n步骤 {step}/{self.total_steps} - "
                    f"匹配集: 损失={matched_loss:.4f}, 准确率={matched_acc:.4f} | "
                    )
