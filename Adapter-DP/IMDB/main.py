...
class DPAdapterTrainer:
    ...
    def _create_val_loader(self):
        """创建验证集数据加载器"""
        # 加载验证集
        val_dataset = load_dataset("imdb", split="train", cache_dir=self.config.cache_dir)
        # IMDB没有预定义验证集，从训练集拆分
        full_train = load_dataset("imdb", split="train", cache_dir=self.config.cache_dir)
        
        # 拆分训练集（80%训练，20%验证）
        split = full_train.train_test_split(test_size=0.2, seed=self.config.seed)
