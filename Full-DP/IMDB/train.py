...
def train_with_dpsgd(model, train_loader, val_loader, optimizer, scheduler, 
    ...
    # 如果训练提前终止，打印最佳指标
    if privacy_budget_reached:
        tqdm.write("Training terminated early due to privacy budget constraints.")
        tqdm.write(f"Final validation accuracy: {val_acc:.4f}")
    
    return history, val_acc, final_epsilon

def evaluate_model(model, data_loader, device):
    """使用 Hugging Face 的 evaluate 库计算评估指标（IMDB专用）"""
    import evaluate
