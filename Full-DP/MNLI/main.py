...
def main():
    ...
    # 1. 准备数据集
    mnli_datasets, tokenizer, collator = prepare_datasets(
        max_length=config["max_length"],
        cache_dir=config["cache_dir"]
    ) 
    
    # 计算采样率
    dataset_size = len(mnli_datasets["train"])
    sample_rate = config["batch_size"] / dataset_size
     
    # 创建训练数据加载器
    train_loader = DPDataLoader(
        dataset=mnli_datasets["train"],
    ...
    val_loader = DataLoader(
        mnli_datasets["validation_matched"], 
    ...
    # 2. 加载模型（注意：num_labels=3）  
    model = RobertaForSequenceClassification.from_pretrained(
        "roberta-base",
        num_labels=3,  # MNLI有三个类别
        cache_dir=config["cache_dir"]
    ).to(device)
    ...
    open("./result.txt", "w", encoding="utf-8").write(f"{final_acc}")
    # 7. 最终评估
    print("\nTraining completed!")
