...
def get_processed_cache_path(config, processor_name):
    ...
    config_hash = hashlib.md5(
        f"{config.dataset_name}_{config.model_name}_{config.max_seq_length}_{processor_name}".encode()
    ).hexdigest()[:8]
...
class GLUEProcessor(DataProcessor):
    ...
    def process_dataset(self, raw_dataset, tokenizer, config):
        ...
        # 获取当前数据集对应的标签列名
        label_col = config.label_columns.get(config.dataset_name, "label")
        ...
        def preprocess_function(examples):
            ...
            if config.dataset_name == "mnli":
                ...
                # IMDB数据集：单句情感分析
                result = tokenizer(
                    examples["text"],  # IMDB的文本字段名为"text"
                    truncation=True,
                    max_length=config.max_seq_length,
                    padding="max_length"
                )
